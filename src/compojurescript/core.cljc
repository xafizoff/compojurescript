...
#?(:cljs
   (do
     ...
     (defn route-compile
       ...
       ([path regexs]
        (clout/route-compile path regexs)))
           ...
           (let [request-method (:request-method request)]
             (case request-method
               ...
               (if-let [fm (form-method request)]
                 (= (.toUpperCase (name method)) (.toUpperCase fm))
                 (= method :post))
               ;else
     ...
     (defn- decode-route-params [params]
       (map-vals js/decodeURIComponent params))
   ...
   (do
     ...
     (defn compile-route
       ...
       `(compojurescript.core/make-route
         ~method
         ~(prepare-route path)
         (fn [request#]
     ...
     (defmacro defroutes
       ...
       (let [[name routes] (macro/name-with-attributes name routes)]
         `(def ~name (compojurescript.core/routes ~@routes))))
     ...
     (defmacro rfn "Generate a route that matches any method and path."
       [args & body]
       `(compojurescript.core/make-rfn (fn [request#] (let-request [~args request#] ~@body))))
     ...
     (defmacro context
       ...
       path prefix ('/user/:id') and a common binding ('id'):
           (context \"/user/:id\" [id]
             ...
             (GET \"/settings\" [] ...))"
       [path args & routes]
       `(compojurescript.core/make-context
         ~(context-route path)
         (fn [request#]
           (let-request [~args request#]
                        (compojurescript.core/routes ~@routes)))))
     ...
     (defmacro let-routes
       ...
       Equivalent to:
           (let [...] (routes ...))"
       [bindings & body]
       `(let ~bindings (compojurescript.core/routes ~@body)))))
